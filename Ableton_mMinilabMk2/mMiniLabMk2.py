...
from _Arturia.ArturiaControlSurface import (
    ...
    LIVE_MODE_MSG_HEAD,
    LOAD_MEMORY_COMMAND,
    MEMORY_SLOT_PROPERTY,
    OFF_VALUE,
    SETUP_MSG_PREFIX,
    SETUP_MSG_SUFFIX,
    STORE_IN_MEMORY_COMMAND,
    WORKING_MEMORY_ID,
    WRITE_COMMAND,
    split_list,
)
...
class mMiniLabMk2(ArturiaControlSurface):
    ...
    def _create_controls(self):
        ...
        if TWO_SENDS:
            ...
            self._send_encoders = ButtonMatrixElement(
                rows=[[self._knob6_encoder, self._knob14_encoder]]
            )
            ...
            self._return_encoders = ButtonMatrixElement(
                rows=[[self._knob7_encoder, self._knob15_encoder]]
            )
        else:
            # * Four sends
            # envios de los send
            ...
            self._send_encoders = ButtonMatrixElement(
                rows=[
                    [
                        self._knob6_encoder,
                        self._knob7_encoder,
                        self._knob14_encoder,
                        self._knob15_encoder,
                    ]
                ]
            )
        ...
        self._knob8_button = ButtonElement(
            True,
            MIDI_CC_TYPE,
            self.button_encoder_msg_channel,
            ENCODER_PUSH2,
            name="Arm_Button",
        )
        ...
        self._switchpad_button = SysexValueControl(
            SWITCHPAD_BUTTON_MSG, name="Pad_Selection_button"
        )
        self._shift_button = SysexValueControl(
            SHIFT_BUTTON_MSG, name="Shift_button_pressed"
        )
    ...
    def _create_session(self):
        self._session = self.session_component_type(
            ...
            is_enabled=False,
            layer=Layer(
                clip_launch_buttons=self._pads, scene_select_control=self._knob8_encoder
            ),
        )
    ...
    def _collect_setup_messages(self):
        for cc_id, encoder_id in zip(self.encoder_msg_ids, HARDWARE_ENCODER_IDS):
            ...
            self._setup_hardware_encoder(
                encoder_id, cc_id, channel=self.encoder_msg_channel
            )
        ...
        for index, pad_id in enumerate(HARDWARE_BUTTON_IDS):
            ...
            self._setup_hardware_button(
                pad_id, index + PAD_IDENTIFIER_OFFSET, self.pad_channel
            )

        self._messages_to_send.append(
            SETUP_MSG_PREFIX
            + (STORE_IN_MEMORY_COMMAND, LIVE_MEMORY_SLOT_ID)
            + SETUP_MSG_SUFFIX
        )
        self._messages_to_send.append(
            SETUP_MSG_PREFIX
            + (LOAD_MEMORY_COMMAND, ANALOG_LAB_MEMORY_SLOT_ID)
            + SETUP_MSG_SUFFIX
        )
    ...
    def _setup_hardware(self):
        ...
        sequence_to_run = [
            Task.run(partial(send_subsequence, subsequence))
            for subsequence in split_list(self._messages_to_send, 20)
        ]
    ...
    def _setup_control_listeners(self):
        ...
        if not self._knob6_encoder.value_has_listener(self._encoder_n5):
            self._knob6_encoder.add_value_listener(
                self._encoder_n5, identify_sender=False
            )
        if not self._knob14_encoder.value_has_listener(self._encoder_n13):
            self._knob14_encoder.add_value_listener(
                self._encoder_n13, identify_sender=False
            )
        if not self._knob7_encoder.value_has_listener(self._encoder_n6):
            self._knob7_encoder.add_value_listener(
                self._encoder_n6, identify_sender=False
            )
        if not self._knob15_encoder.value_has_listener(self._encoder_n14):
            self._knob15_encoder.add_value_listener(
                self._encoder_n14, identify_sender=False
            )
        if not self._pan_encoder.value_has_listener(self._encoder_n7):
            self._pan_encoder.add_value_listener(
                self._encoder_n7, identify_sender=False
            )
        if not self._volume_encoder.value_has_listener(self._encoder_n15):
            self._volume_encoder.add_value_listener(
                self._encoder_n15, identify_sender=False
            )

        # _knob0_encoder
        if not self._knob0_encoder.value_has_listener(self._encoder_n0):
            self._knob0_encoder.add_value_listener(
                self._encoder_n0, identify_sender=False
            )
        if not self._knob0_button.value_has_listener(self._encoder_n0_button):
            self._knob0_button.add_value_listener(
                self._encoder_n0_button, identify_sender=False
            )

        if not self._knob8_encoder.value_has_listener(self._encoder_n8):
            self._knob8_encoder.add_value_listener(
                self._encoder_n8, identify_sender=False
            )
        if not self._knob8_button.value_has_listener(self._encoder_n8_button):
            self._knob8_button.add_value_listener(
                self._encoder_n8_button, identify_sender=False
            )

        if not self._switchpad_button.value_has_listener(
            self._switchpad_value_on_press
        ):
            self._switchpad_button.add_value_listener(
                self._switchpad_value_on_press, identify_sender=False
            )

        if not self._shift_button.value_has_listener(self._shift_value_on_press):
            self._shift_button.add_value_listener(
                self._shift_value_on_press, identify_sender=False
            )
        ...
        if self.shift_active or self.modo_clip_activo:
            ...
            if value > 0:
                if self.application().view.is_view_visible("Detail/Clip"):
                    ...
                    self.Actions.enc_moveinto_devices(
                        value, self.__selecteddevice, self.__sub_devices
                    )
        ...
        if self.shift_active or self.modo_clip_activo:
            ...
            if value > 0:
                if self.application().view.is_view_visible("Detail/Clip"):
                    ...
                    track = self.song_instance.view.selected_track
                    if not track.view.selected_device_has_listener(
    ...
    def get_session_track(self, pad_id):
        ...
        if track_number < len(total_v_tracks):
    ...
    def device_properties(self, device):
        ...
        if can_have_chains:
            ...
            logger.info(
                "device is a Rack :: can_show_chains :: " + str(can_show_chains)
            )
            is_showing_chain_devices = device.view.is_showing_chain_devices
            logger.info(
                "device is a Rack :: is showing chain devices :: "
                + str(is_showing_chain_devices)
            )
            ...
            logger.info(
                "device is a Rack :: chain selected name :: " + str(chain_selected_name)
            )
            chain_devices = chain_selected.devices
            if chain_devices:
                chain_devices_total = len(chain_devices)
                logger.info(
                    "device is a Rack :: chain selected devices count :: "
                    + str(chain_devices_total)
                )
                if chain_devices_total > 0:
                    for ch_device in chain_devices:
                        ch_dev_name = ch_device.name
                        logger.info(
                            "device is a Rack :: chain selected devices name :: "
                            + str(ch_dev_name)
                        )
