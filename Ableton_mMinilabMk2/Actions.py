...
class Actions(object):
        ...
        if track.devices:
            ...
            if device_sel in devices_tot:
                ...
                if value > 64:
                    ...
                    self.device_collapsed(
                        device_sel, prev_device, IN_DEVICE_VIEW_COLLAPSED
                    )
                    ...
                    if dev_index + 1 < len(devices_tot):
                        ...
                        self.device_collapsed(
                            device_sel, next_device, IN_DEVICE_VIEW_COLLAPSED
                        )
    ...
    def device_collapsed(
        self,
        device_sel,
        prev_or_next_device,
        device_collapsed=False,
        device_excluded=None,
    ):
    ...
    def _nav_view_leftright(self, value, visible_view):
        ...
        if value > 64:
            self.application().view.scroll_view(
                self.application().view.NavDirection.left, visible_view, False
            )
        else:
            self.application().view.scroll_view(
                self.application().view.NavDirection.right, visible_view, False
            )
    ...
    def _nav_view_updown(self, value, visible_view):
        ...
        if value > 20:
            self.application().view.scroll_view(
                self.application().view.NavDirection.up, visible_view, False
            )
        else:
            self.application().view.scroll_view(
                self.application().view.NavDirection.down, visible_view, False
            )
    ...
    def enc_set_gridquant_value(self, value):
        ...
        clip = self.song_instance.view.detail_clip
        if clip and self.application().view.is_view_visible("Detail/Clip"):
            qg_actual_index = 0
            ...
            logger.info(
                "GridQuantization : "
                + str(clip.view.grid_quantization)
                + " - index: "
                + str(qg_actual_index)
            )
            clip.view.show_loop()
    ...
    def _enc_seldevice_preset(self, value):
        ...
        if presets is not None:
            ...
            if value > 64:
                ...
                message = (
                    str(appo_device.name)
                    + " < "
                    + str(actual_preset)
                    + " - "
                    + str(presets[actual_preset])
                )
                return message
                # self.show_message(
